...
let command: Command = {
    ...
    run: (message, client, args) =>
    {
        ...
        if (!perms) return message.channel.send("<a:error:735244847019065416> **| No tienes Permisos necesarios**").then(m =>
        {
        ...
        try
        {
            ...
            if (!user) return message.channel.send('<a:error:735244847019065416> **| Debe Mencionar a un Usuario**').then(m =>
            {
                setTimeout(() => { m.delete() }, 5000)
            })

            if (user === message.author)
                return message.channel.send("<a:error:735244847019065416> **| No te puedes expulsar a ti mismo!.** ")
            ...
            if (!razon) return message.channel.send('<a:error:735244847019065416> **| Razón no escrita**').then(m =>
            {
                setTimeout(() => { m.delete() }, 5000)
            })
        ...
        {
            message.channel.send(`**Hubo un error, porfavor si el error continua comuniquelo a nuestro servidor de soporte\n\nError:** ||\`${er}\`||`)
        }
    }
}
